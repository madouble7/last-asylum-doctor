...
- Durable architecture orientation lives in `docs/architect_state.md` and `docs/architecture_decisions.md`; ARCHITECT should read them with relevant current repository evidence before major planning. ENGINEER is the specialist/experimental role in the VS Code Agents window and does not replace the standing ARCHITECT, ATLAS, SCOUT, or PROBE roles.
- The ATLAS INTEGRATOR should update `docs/architect_state.md` during a checkpoint only when production capability, architectural topology, major evidence posture, policy constraints, or the active milestone materially changes. Do not update it for trivial implementation details. Add a new ADR only for a genuinely consequential decision.
